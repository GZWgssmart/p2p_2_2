...
<div class="news cl">
    <div class="news-contents" style="display: block;">
        <h3 id="news-title">{{media.title}}</h3>
        <div class="news-contents-top">
            ...
            <a class="news-back fr" href="<%=path %>/index/mtbd">返回列表</a>
            <p class="news-time fr">{{media.date | date:'yyyy-MM-dd HH:mm:ss'}}</p>
        </div>
...
<script type="text/javascript">
    ...
    (function ($) {
        ...
        app.controller('myCtrl', function ($scope, $http) {
            $http({
                method: 'POST',
                url: "/media/single_media",
                data:{mid:mediaId}
            }).then(function successCallback(response) {
                $scope.media = response.data;// 数据
                $scope.myText = $scope.media.content;
