...
//        		$('#userTotal').text(data.totalUserMap.userTotal);
                ...
                if(data.mapList6 != null){
	        		...
	        		$('#xs-schedules').text(data.mapList6.schedules);
	        		$('#ty-date').text(data.mapList6.deadline+'个月');
	        		$('#xs-doll').text(data.mapList6.borrowAmount);
	        		$('.progress em').css("width",data.mapList6.schedules+'%');
        		...
        		if (data.isNewInvest == 1) {
        			var btnText = getStatus(data.mapList6.borrowStatus);
        			$("#ty-btn").text(btnText);
    				$("#ty-btn").css("background-color", "#b0b0b0");
    				$("#ty-btn").attr("disabled",true);
        		}
        		var btnText = getStatus(data.mapList6.borrowStatus);
    			$("#ty-btn").text(btnText);
    			if(data.mapList6.borrowStatus != '2'){
    				$("#ty-btn").css("background-color", "#b0b0b0");
    				$("#ty-btn").attr("disabled",true);
    			}
        		//
...
//        		});
        		...
        		//债权转让
        		$("#debtBtn").bind('click',function(){
            		if(data.error==1){
						utils.alert('您未登录！请先登录',function(){									
							window.location.href='../login.html';
						});
						return;
        			}else{
        				window.location.href='account.html#claimm';
        			}	
        		});
            	var debt = $('#product-debt');
            	var debtList = data.mapList8;           
            	if(debtList.length>0){
            		for(var i = 0;i<debtList.length;i++){
            			var allTotla = parseFloat(debtList[i].debtSum)+parseFloat(debtList[i].recivedInterest);
            			var debtLi=	$('<li><p><a class="creditor-a" href="creditor.html?id='+debtList[i].id+'">'+debtList[i].borrowTitle+'</a></p>'+
						'<p class="p-red">'+debtList[i].showRate+'%</p><p>'+debtList[i].debtLimit+'/'+debtList[i].deadline+'</p>'+
						'<p>'+utils.formatCurrency(allTotla)+'</p><p>'+utils.formatCurrency(debtList[i].auctionBasePrice)+'</p>'+
						'<p><a class="button" href="creditor.html?id='+debtList[i].id+'">立即认购</a></p></li>')
						debtLi.appendTo(debt);	
            		}
            	}else{
            		$('#pj_debt').remove();
            	}
